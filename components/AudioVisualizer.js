...
import visualizerData from "../assets/data/audio_visualizer_data.json";
...
const AudioVisualizer = () => {
   ...
   useEffect(() => {
      const loadSound = async () => {
         ...
         const { sound } = await Audio.Sound.createAsync(
            require('../assets/audio/DJ SHONE FT. ANABELA & ELITNI ODREDI - BEOGRAD.mp3')
         );
   ...
   useEffect(() => {
      const interval = setInterval(async () => {
         ...
         if (status.isLoaded && status.isPlaying) {
            ...
            setFrameIndex(frameIndex % visualizerData.length);
         }
      }, 25); 
   ...
   const frame = visualizerData[frameIndex];
   const progress = position / duration;

   return (
      <View style = {{position: 'relative', backgroundColor: 'pink', height: '100%'}}>
         {/* Favorite button */}
         <TouchableOpacity onPress={toggleFavorite} style={styles.favoriteIcon}>
            <FontAwesome name={isFavorite ? "heart" : "heart-o"} size={26} color={isFavorite ? "#e43359" : "white"} />
         </TouchableOpacity>
            ...
            <View style={styles.playerControls}>
               ...
               <View style={styles.controlButtons}>
                  ...
                  <TouchableOpacity onPress={handlePlayPause}>
                     <FontAwesome name={isPlaying ? "pause" : "play"} size={27} color="white" style = {{marginLeft: !isPlaying ? '1.7' : '0', marginRight: !isPlaying ? '0.6' : '0'}} />
                  </TouchableOpacity>
